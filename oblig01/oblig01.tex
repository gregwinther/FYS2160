...
\lstinputlisting[firstline=4, lastline=28]{scripts/EinsteinCrystal.py}
...
\lstinputlisting[firstline=31, lastline=43]{scripts/EinsteinCrystal.py}
...
\lstinputlisting[firstline=46, lastline=61]{scripts/EinsteinCrystal.py}
...
It would be interesting to see how this approximation compares with a numberical approximation of a similar situation. The following class written in Python provides a framework where $M$ microstates are generated randomly for a system of $N$ dipoles. 

\lstinputlisting[firstline=5, lastline=50]{scripts/RandomSpin.py}

Additionally, the class includes two simple set-methods, in order to change the value for $\mu$ and $B$. An object of this class can be made with for example $M=10000$ and $N=50$. Running the \lstinline$energyHistogram(binsNumber)$ method will render a figure that compares the randomly generated spin systems to the analytical version in equation \ref{eq:omegazero}, shown in figure \ref{fig:randomspin}.
...
Now, we will return to describing the system using exact, and not the approximate, multiplicity. The \emph{entropy} $S$ as a function of $N$ and $N_{\uparrow}$ will, using Stirling's approximation, be
\begin{align}
